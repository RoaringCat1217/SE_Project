    ...
    <div>
        ...
        <div class="reply">
            <div class="reply-list">
                <el-card
                    v-for="(comment, index) in commentList.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
                    :key="index">
                    <template #header>
                        <div class="card-header">
                    ...
                    <el-container>
                        ...
                        <el-container>
                            ...
                            <el-footer>
                                ...
                                <div class="like-star">
                                    <button class="reply-button" @click="handleReply(comment.reply_id)">
                                        <el-icon :size="20">
                                            <Edit />
                                        </el-icon>
                                        回帖
                ...
                <el-dialog v-model="dialogVisible" title="快速回帖" center>
                    <el-input v-model="reply_content" maxlength="1000" :rows="10" placeholder="请输入回复的内容" show-word-limit
                        type="textarea" />
                    <template #footer>
                ...
                <div id="pagination">
                    <el-pagination background v-model:current-page="currentPage" v-model:page-size="pageSize"
                        layout="prev, pager, next,jumper,-> ,total" :total="commentList.length">
                    </el-pagination>
                </div>
            </div>
        </div>
        ...
        <div class="new-reply" v-if="isLogin">
            <p>回复主贴</p>
            <el-form ref="replyRef" :model="replyInfo" label-width="50px" :rules="commitRules">
                <el-form-item label="内容" prop="content">
                    <el-input v-model="replyInfo.content" :rows="10" type="textarea" placeholder="请输入帖子正文内容"
    ...
    api.getPostDetails(post_params)
        .then((res) => {
            if (res.data.code === 1) {
                ...
                api.getUserAvatar({ username: post_info.poster_name })
                    .then(res => {
                        if (res.data.code === 1) {
                            if (res.data.image == '') {
        ...
        .then((res) => {
            if (res.data.code === 1) {
                ...
                for (let i in res.data.entry) {
                    ...
                    api.getUserAvatar({ username: poster_id }).then(
                        res => {
                            // 头像显示
                            if (res.data.code === 1) {
                                if (res.data.image == '') {
                                    ...
                                    commentList.value[i]['replyer_avatar'] = res.data.image.replace(/\s/g, '+');
                                }
                            }
